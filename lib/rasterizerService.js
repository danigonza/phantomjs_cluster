...
RasterizerService.prototype.startService = function(callback) {
  var rasterizer = spawn(this.config.command, ['rasterizer.js', this.port, this.config.viewport]);
  var self = this;
  rasterizer.stderr.on('data', function (data) {
    self.clim.console.error("Phantomjs server in port " + self.getPort() + " error: " + data.toString().trim());
  });
  rasterizer.stdout.on('data', function (data) {
    self.clim.console.log("Phantomjs server in port " + self.getPort() + " output: " + data.toString().trim());
  });
...
RasterizerService.prototype.killService = function() {
  if (this.rasterizer) {
    ...
    this.clim.console.log("Stopping Phantomjs internal server in port " + this.getPort());
  }
}
  ...
  request('http://localhost:' + this.port + '/healthCheck', function(error, response) {
    if (error || response.statusCode != 200){
      if (self.config.debug){
        self.clim.console.error("Error in Pong for server %s", port);
      }
      return;
    }
    if (self.config.debug){
      self.clim.console.info("Pong for server %s", port);
    }
