...
RasterizerService.prototype.startService = function() {
  ...
  rasterizer.stderr.on('data', function (data) {
    console.error("Phantomjs server "/*in port " + this.getPort()*/ + " error: " + data.toString().trim());
  });
  rasterizer.stdout.on('data', function (data) {
    console.log("Phantomjs server "/*in port " + this.getPort()*/ + " output: " + data.toString().trim());
  });
...
RasterizerService.prototype.pingService = function() {
  ...
  if (this.config.debug){
    console.info("Ping for server %s", port);
  } 
  request('http://localhost:' + port + '/healthCheck', function(error, response) {
    if (error || response.statusCode != 200){
      if (self.config.debug){
        console.error("Error in Pong for server %s", port);
      }
      return;
    }
    if (self.config.debug){
      console.info("Pong for server %s", port);
    }
...
RasterizerService.prototype.setLastHealthCheckDate = function(date, port) {
  if (this.config.debug){
    console.warn("Set lastHealthCheckDate %s for server " + port, date);
  } 
