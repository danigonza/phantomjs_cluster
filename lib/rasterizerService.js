...
RasterizerService.prototype.startService = function() {
  ...
  rasterizer.stderr.on('data', function (data) {
    this.clim.console.error("Phantomjs server in port " + self.getPort() + " error: " + data.toString().trim());
  });
  rasterizer.stdout.on('data', function (data) {
    this.clim.console.log("Phantomjs server in port " + self.getPort() + " output: " + data.toString().trim());
  });
